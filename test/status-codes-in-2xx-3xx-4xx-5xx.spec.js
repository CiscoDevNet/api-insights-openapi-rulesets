/**
 * Copyright 2022 Cisco Systems, Inc. and its affiliates.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *      http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 *
 * SPDX-License-Identifier: Apache-2.0
 */

import fsPromises from 'fs/promises';
import path from 'path';
import { prepLinter } from '../util/testUtils';
import ruleset from '../api-insights-openapi-ruleset';
const ruleName = 'status-codes-in-2xx-3xx-4xx-5xx';
const resPath = path.join(__dirname, `resources/${ ruleName }`);

describe(ruleName, () => {
  let spectral;

  beforeAll(() => {
    spectral = prepLinter(ruleset, ruleName);
  });
  test('should catch invalid response codes in all operations', async () => {
    const spec = await fsPromises.readFile(`${ resPath }/negative-all-operations.yml`);
    const res = await spectral.run(spec.toString());

    expect(res).toEqual([
      {
        code: ruleName,
        message: 'API responds with recommended HTTP status codes in the 2xx/3xx/4xx/5xx ranges.; Status Code(s) [110] must be in the 2xx/3xx/4xx/5xx ranges',
        path: [
          'paths',
          '/test',
          'get',
          'responses',
        ],
        range: {
          start: {
            line: 19,
            character: 16,
          },
          end: {
            line: 23,
            character: 28,
          },
        },
        severity: 0,
      },
      {
        code: ruleName,
        message: 'API responds with recommended HTTP status codes in the 2xx/3xx/4xx/5xx ranges.; Status Code(s) [110] must be in the 2xx/3xx/4xx/5xx ranges',
        path: [
          'paths',
          '/test',
          'put',
          'responses',
        ],
        range: {
          start: {
            line: 29,
            character: 16,
          },
          end: {
            line: 33,
            character: 28,
          },
        },
        severity: 0,
      },
      {
        code: ruleName,
        message: 'API responds with recommended HTTP status codes in the 2xx/3xx/4xx/5xx ranges.; Status Code(s) [110] must be in the 2xx/3xx/4xx/5xx ranges',
        path: [
          'paths',
          '/test',
          'post',
          'responses',
        ],
        range: {
          start: {
            line: 39,
            character: 16,
          },
          end: {
            line: 43,
            character: 28,
          },
        },
        severity: 0,
      },
      {
        code: ruleName,
        message: 'API responds with recommended HTTP status codes in the 2xx/3xx/4xx/5xx ranges.; Status Code(s) [110] must be in the 2xx/3xx/4xx/5xx ranges',
        path: [
          'paths',
          '/test',
          'delete',
          'responses',
        ],
        range: {
          start: {
            line: 49,
            character: 16,
          },
          end: {
            line: 53,
            character: 28,
          },
        },
        severity: 0,
      },
      {
        code: ruleName,
        message: 'API responds with recommended HTTP status codes in the 2xx/3xx/4xx/5xx ranges.; Status Code(s) [110] must be in the 2xx/3xx/4xx/5xx ranges',
        path: [
          'paths',
          '/test',
          'options',
          'responses',
        ],
        range: {
          start: {
            line: 59,
            character: 16,
          },
          end: {
            line: 63,
            character: 28,
          },
        },
        severity: 0,
      },
      {
        code: ruleName,
        message: 'API responds with recommended HTTP status codes in the 2xx/3xx/4xx/5xx ranges.; Status Code(s) [110] must be in the 2xx/3xx/4xx/5xx ranges',
        path: [
          'paths',
          '/test',
          'head',
          'responses',
        ],
        range: {
          start: {
            line: 69,
            character: 16,
          },
          end: {
            line: 73,
            character: 28,
          },
        },
        severity: 0,
      },
      {
        code: ruleName,
        message: 'API responds with recommended HTTP status codes in the 2xx/3xx/4xx/5xx ranges.; Status Code(s) [110] must be in the 2xx/3xx/4xx/5xx ranges',
        path: [
          'paths',
          '/test',
          'patch',
          'responses',
        ],
        range: {
          start: {
            line: 79,
            character: 16,
          },
          end: {
            line: 83,
            character: 28,
          },
        },
        severity: 0,
      },
      {
        code: ruleName,
        message: 'API responds with recommended HTTP status codes in the 2xx/3xx/4xx/5xx ranges.; Status Code(s) [110] must be in the 2xx/3xx/4xx/5xx ranges',
        path: [
          'paths',
          '/test',
          'trace',
          'responses',
        ],
        range: {
          start: {
            line: 89,
            character: 16,
          },
          end: {
            line: 93,
            character: 28,
          },
        },
        severity: 0,
      },
    ]);
  });
  test('should catch 3xx response codes', async () => {
    const spec = await fsPromises.readFile(`${ resPath }/negative.yml`);
    const res = await spectral.run(spec.toString());

    expect(res).toEqual([
      {
        code: ruleName,
        message: 'API responds with recommended HTTP status codes in the 2xx/3xx/4xx/5xx ranges.; Status Code(s) [110] must be in the 2xx/3xx/4xx/5xx ranges',
        path: [
          'paths',
          '/test',
          'get',
          'responses',
        ],
        range: {
          start: {
            line: 19,
            character: 16,
          },
          end: {
            line: 23,
            character: 28,
          },
        },
        severity: 0,
      },
    ]);
  });
  test('should pass 2xx, 4xx, and 5xx response codes', async () => {
    const spec = await fsPromises.readFile(`${ resPath }/positive.yml`);
    const res = await spectral.run(spec.toString());

    expect(res).toEqual([]);
  });
});
