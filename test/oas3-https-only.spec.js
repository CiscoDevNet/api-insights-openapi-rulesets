...
const ruleName = 'oas3-https-only';
...
describe(ruleName, () => {
  ...
  test('should detect if protocols are not HTTPS', async () => {
    ...
    expect(res).toEqual([
      {
        code: ruleName,
        message: 'My API supports HTTPS only.; "http://api.example.com/v1" must match the pattern "^https://"',
        path: [
          'servers',
          '0',
          'url',
  ...
  test('should pass if only HTTPS is used', async () => {
