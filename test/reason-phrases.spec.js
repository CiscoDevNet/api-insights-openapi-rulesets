...
describe(ruleName, () => {
  ...
  test('should throw an error if any of the reason phrases do not match', async () => {
    const spec = await fsPromises.readFile(`${ resPath }/negative.yml`);
    const res = await spectral.run(spec.toString());

    expect(res.length).toBeTruthy();
    // for the sake of brevity and my sanity, just check the beginning
    for (const r of res) {
      const re = /Reason phrase ".*" needs to match ".*"/;

      expect(re.test(r.message)).toEqual(true);
    }
  });
