...
const ruleName = 'oas3-request-header-accept-charset-valid-enum';
...
describe(ruleName, () => {
  ...
  test('should fail a request Accept-Charset header where the allowed values do not include "utf-8" and "iso-8859-1"', async () => {
  ...
  test('should pass a request Accept-Charset header enum that includes both "utf-8" and "iso-8859-1" and only those two', async () => {
    const spec = await fsPromises.readFile(`${ resPath }/positive.yml`);
    const res = await spectral.run(spec.toString());

    expect(res).toEqual([]);
  });
  test('should pass a request Accept-Charset header enum that includes both "utf-8" and "iso-8859-1" and others', async () => {
    const spec = await fsPromises.readFile(`${ resPath }/positive-others.yml`);
    const res = await spectral.run(spec.toString());
