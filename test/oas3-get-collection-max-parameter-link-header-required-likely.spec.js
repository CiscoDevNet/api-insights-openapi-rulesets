...
const ruleName = 'oas3-get-collection-max-parameter-link-header-required-likely';
...
describe(ruleName, () => {
  ...
  test('should not flag a collection where it has multiple child arrays (not confident)', async () => {
    const spec = await fsPromises.readFile(`${ resPath }/positive-object-multi-array.yml`);
    const res = await spectral.run(spec.toString());

    expect(res).toEqual([]);
  });
  test('should only flag missing max query parameter if Link header is present', async () => {
  ...
  test('should only flag missing Link header if max query parameter is present', async () => {
  ...
  test('should not flag a response that already has a max query parameter and a Link header', async () => {
    const spec = await fsPromises.readFile(`${ resPath }/positive-both-present.yml`);
    const res = await spectral.run(spec.toString());
