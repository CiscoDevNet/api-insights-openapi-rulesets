...
export default function (request, _opts, context) {
  ...
  // fullRep means the request body schema and the response schema are equivalent,
  // or are both undefined
  const fullRep = (clonedResponses['200']?.content && _.isEqual(clonedResponses['200']?.content, requestBody?.content))
        || (clonedResponses['200']?.schema && _.isEqual(clonedResponses['200']?.schema, requestBodySchema))
    ...
    // check for 200 Ok and full representation
  const validOk = clonedResponses['200'] && fullRep && clonedResponses['200'].description;
